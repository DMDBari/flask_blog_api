...
def create_post():
    ...
    for field in required_fields:
        ...
        if field not in data:
            ...
            missing_fields.append(field)
    # If there are any missing fields, return 400 status code with the missing fields
    if missing_fields:
        return {'error': f"{', '.join(missing_fields)} must be in the request body"}, 400
...
def delete_post(post_id):
    # based on the post_id parameter check to see if Post exists
    post = db.session.get(Post, post_id)
    if post is None:
        return {'error': f"Post with ID #{post_id} does not exist"}, 404
    # Get the current user based on the token
    ...
    # delete the post
    post.delete()
    return {'success': f"{post.title} was successfully deleted"}
